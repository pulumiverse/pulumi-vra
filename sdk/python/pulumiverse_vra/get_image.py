...
def get_image(filter: Optional[str] = None,
    ...
    """
    The `get_image` data source can be used to discover the lookup machine images with cloud accounts. This can then be used with resource that require an image. For example, to create an image profile using the `ImageProfile` resource.
    ...
    this_cloud_account_vsphere = vra.get_cloud_account_vsphere(name=var["cloud_account"])
    this_region = vra.get_region(cloud_account_id=this_cloud_account_vsphere.id,
        region=var["region"])
    image0 = vra.get_image(filter=f"name eq '{var['image_name_0']}' and cloudAccountId eq '{this_cloud_account_vsphere.id}' and externalRegionId eq '{var['region']}'")
    image1 = vra.get_image(filter=f"name eq '{var['image_name_1']}' and cloudAccountId eq '{this_cloud_account_vsphere.id}' and externalRegionId eq '{var['region']}'")
    this_image_profile = vra.ImageProfile("thisImageProfile",
    ...
    __ret__ = pulumi.runtime.invoke('vra:index/getImage:getImage', __args__, opts=opts, typ=GetImageResult).value
