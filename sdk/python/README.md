...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `vra` with the `NAME` of your provider.
...
- binary: pulumi-resource-vra
  ...
  ignore: []
  ldflags:
  - -X github.com/pulumi/pulumi-vra/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-vra/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-vra_darwin_amd64_v1
│   └── pulumi-resource-vra
├── pulumi-vra_darwin_arm64
│   └── pulumi-resource-vra
├── pulumi-vra_linux_amd64_v1
│   └── pulumi-resource-vra
├── pulumi-vra_linux_arm64
│   └── pulumi-resource-vra
├── pulumi-vra_windows_amd64_v1
│   └── pulumi-resource-vra.exe
└── pulumi-vra_windows_arm64
    └── pulumi-resource-vra.exe
```
